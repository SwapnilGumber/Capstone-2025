...
const changeAdmin = async (req, res) => {
	try {
		...
		// Find current admins
		const currentAdmins = team.members.filter(
			(member) => member.role === "team_admin"
		);
		...
		// Demote current admins if needed (optional: you can allow multiple admins)
		// Here, we keep at least one admin: the newAdmin
		currentAdmins.forEach((admin) => {
			admin.role = "member";
			admin.accessLevel = "read"; // Optional: reset accessLevel
		});
